...
**完全复用现有技术栈**，仅新增 FFmpeg 用于关键帧提取：

```
视频文件 → FFmpeg提取关键帧 → CN-CLIP编码 → Faiss索引 → 搜索结果
                ↓                    ↓
          内存图片(PIL)        特征向量(512维)
                ↓                    ↓
          无需临时文件         与图片向量混合索引
...
**核心流程**：
- **索引构建**：视频 → 按间隔提取关键帧（默认10秒） → CN-CLIP编码 → 存入Faiss
- **搜索阶段**：图片查询 → CN-CLIP编码 → Faiss搜索 → 返回图片+视频帧结果
- **元数据管理**：新增 `video_frames` 表存储时间戳和向量ID映射

### 9.2 新增技术选型

| 技术 | 用途 | 选择理由 |
|------|------|---------|
| FFmpeg | 视频关键帧提取 | 性能更优（快2倍）、无需临时文件、格式兼容性强 |
...
CREATE TABLE video_frames (
    ...
    video_id INTEGER NOT NULL,           -- 关联视频文件ID
    timestamp INTEGER NOT NULL,           -- 时间戳（秒）
    vector_id INTEGER NOT NULL,           -- Faiss向量ID
    file_id INTEGER NOT NULL,             -- 关联files表ID
    frame_width INTEGER,                  -- 帧宽度
    frame_height INTEGER,                 -- 帧高度
...
class VideoFrameConfig(BaseSettings):
    video_frame_search_enabled: bool = False
...
**修改接口**：`POST /api/search/multimodal`
...
**新增组件**：`VideoFrameResultCard.vue`

**核心功能**：
- 视频图标标识（`VideoCameraOutlined`）
- 关键帧预览图显示
- 时间戳展示（HH:MM:SS格式）
- 相似度分数圆形进度条
- 操作按钮：打开文件、复制路径、跳转到位置（可选）

### 9.7 核心模块

**新增服务**：`backend/app/services/video_frame_extractor.py`
```python
# 基于 FFmpeg 的实现
import ffmpeg
...
class VideoFrameExtractor:
    ...
    async def extract_frames(
        ...
        """
        提取视频关键帧到内存（无需临时文件）
        使用FFmpeg直接提取到PIL Image对象
        """
        # FFmpeg提取帧到内存 → 转换为PIL Image → 返回
        # 性能：快2倍，无磁盘I/O
```
...
**扩展服务**：
- `ChunkIndexService._build_clip_image_index()` - 添加视频帧索引逻辑
- `SearchService.multimodal_search()` - 查询视频帧元数据

### 9.8 性能指标

| 指标 | 目标值 | 说明 |
|-----|-------|------|
| 关键帧提取速度 | ≥1帧/秒 | 10分钟视频≤10秒 |
| 搜索响应时间 | <500ms | 与图搜图一致 |
| 索引构建增量 | <30% | 启用功能后时间增加 |
