...
### 📊 总体统计
- **总接口数**: 38个 *(系统运行状态API 1个，设置管理API 8个，实时消息API 4个，全量重建索引API 1个)*
- **已实现**: 36个 (95%)
...
- **规划中**:
  ...
  - 云端嵌入模型调用能力 (v3.5) 🚧 ✨
  - 全量重建索引接口 (v3.5) 🚧 ✨
- **实现完成度**: 95%（基础功能100%，视频画面搜索已暂停，插件化架构、OpenAI大模型、云端嵌入模型和全量重建索引规划中）
...
**最后更新时间**: 2026年3月26日
**更新内容**: v3.5 云端嵌入模型调用能力 - 新增LocalEmbeddingConfig和CloudEmbeddingConfig接口，支持OpenAI/DeepSeek/阿里云/Moonshot嵌入模型，新增全量重建索引接口POST /api/index/rebuild-all（复用现有索引任务系统），新增6个云端嵌入模型专用错误码（含重建相关）

---

**维护团队**: 小遥搜索开发组
**技术支持**: [GitHub Issues](https://github.com/your-repo/issues)
**文档版本**: v3.5.0 (云端嵌入模型调用能力版)
**文档更新**: 2026-03-26